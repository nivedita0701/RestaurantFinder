...
exports.getRestaurants = async (req, res) => {
  ...
  try {
    ...
    if (latitude && longitude) {
      ...
      restaurants = await Restaurant.findAll({
        ...
        attributes: [
          ...
          [
            sequelize.literal(`
              CASE
                WHEN "ratingsCount" > 0 THEN "totalRatings"::float / "ratingsCount"
                ELSE 0
              END
            `),
            'averageRating',
      ...
      restaurants = await Restaurant.findAll({ where: filters, attributes,
        order: [['name', 'ASC']] });
    }
    ...
    if (rating) {
      ...
      restaurants = restaurants.filter((restaurant) => {
        const averageRating =
          restaurant.totalRatings && restaurant.ratingsCount
            ? restaurant.totalRatings / restaurant.ratingsCount
            : 0;
